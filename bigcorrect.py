...
def kmeans(k, xs, l, n_iter=1):
    ...
    for i in random.sample(xrange(len(xs)), k):
	    select.append(i);
	    init.centers.append(densify(xs[i],l));
	    #print i	
    cluster = [None] * len(xs)
...
#				os.system("curl -XGET 'http://localhost:9200/kmeans/tweet/_search?q=name:\""+word+"\"' > deed.txt");
				res=es.search(index="bigone", body={"query": {"match": {"content": word}}})
#f=open("deed.txt","r")
#	x=f.read()
...
#					print entry
					try:
#		print maptweet
						tweetnumber=int(maptweet[int(ids)])
...
#	print "MAPTWEET IDSS",maptweet
        for j, c in enumerate(init.centers):
            members = (x for i, x in enumerate(xs) if cluster[i] == j)
            init.centers[j] = mean(members, l)
...
#	print row[2]
	row[0]=i;
        data.append( hasher( zip( header, row ),i ) )
	args.append(row[0])
	...
	for w in re.findall(r"\w+", row[2]):
                plen=len(init.vocab);
		init.vocab.setdefault(w, len(init.vocab))
		nlen=len(init.vocab)
		if(plen<nlen):
			worddict[plen]=w;
                x[init.vocab[w]] += 1
...
#	res=es.search(index="new", body={"query": {"match": {"content": row[2]}}})
#	print res	
	mainn.append(key)
        i=i+1
#	print "x==",x
...
# print maptweet
    ...
    cluster_ind = kmeans(5, xs, len(init.vocab))
#    print cluster_ind
...
#   print clusters
    for j, c in enumerate(clusters):
        ...
        for i in c:
            print("\t%s" % args[i])
#    print init.select
#    for i in init.select:
#	res=es.search(index="bigone", body={"query": {"match": {"title":i}}})
...
def funcmain(querys):
	    ...
	    xs=init.xs
		    ...
		    x[init.vocab[w]] += 1
		       ...
		       print 
	    minn=dist(x.items(),init.centers[0]);
	    minid=0;
	    for m in range(1,k):
		    dd=dist(x.items(),init.centers[m]);
		    if(minn<dd):
			minn=dd;
			minid=m;
	    print "DISTANCE",minn
	    ...
	    times=0;
	    ss={}
	    for i in init.clusters[minid]:
	    	if times>50:
			break;
		res=es.search(index="bigone", body={"query": {"match": {"title":i}}})
		ss[i]=res['hits']['hits'][0]['_source']['content']
		times=times+1;
	    return ss
#	          Tweet_Data(mainn[i])
